...
class customer_register_form(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ['username', 'first_name', 'last_name',
                  'email', 'phone', 'gender', 'dob', 'password']
    ...
    def clean_password(self):
        ...
        if len(re.findall('[a-z]', pwd)) == 0:
            raise forms.ValidationError(
                'password atleast 1 lowercase character')
        return pwd
    ...
    def clean_repassword(self):
        ...
        if len(pwd) < 3:
            raise forms.ValidationError(
                'repassword should be min 3 characters')
        if len(pwd) > 15:
            raise forms.ValidationError(
                'repassword should be max 15 characters')
        if len(re.findall('[0-9]', pwd)) == 0:
            raise forms.ValidationError(
                'repassword atleast 1 numeric character')
        if len(re.findall('[^0-9a-zA-Z]', pwd)) == 0:
            raise forms.ValidationError(
                'repassword atleast 1 special character')
        if len(re.findall('[a-z]', pwd)) == 0:
            raise forms.ValidationError(
                'repassword atleast 1 lowercase character')
        return pwd
...
class customer_login_form(forms.Form):
    ...
    password = forms.CharField(widget=forms.PasswordInput(
        attrs={'class': 'form-control border-2 border-secondary w-100 py-2 px-4 rounded-pill', 'placeholder': "Password"}))
...
class change_pwd_form(forms.Form):
    ...
    def clean_re_enter_password(self):
        ...
        if not (password[0].isupper()):
            raise forms.ValidationError(
                're entered password starts with uppercase')
        if len(password) < 3:
            raise forms.ValidationError(
                're entered password should be min 3 characters')
        if len(password) > 15:
            raise forms.ValidationError(
                're entered password should be max 15 characters')
        if len(re.findall('[0-9]', password)) == 0:
            raise forms.ValidationError(
                're entered password atleast 1 numeric character')
        if len(re.findall('[^0-9a-zA-Z]', password)) == 0:
            raise forms.ValidationError(
                're entered password atleast 1 special character')
        if len(re.findall('[a-z]', password)) == 0:
            raise forms.ValidationError(
                're entered password atleast 1 lowercase character')
